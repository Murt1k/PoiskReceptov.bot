...
def _search_from_product(words: list[str]) -> list:
    """
    :param words: Список слов, которые должны быть в блюде

    :return : Список слов, которые должны быть в блюде в подготовленном для
      запроса виде
    """
    words_new = []
    for i in words:
...
def search_worker(words=[], type_dish="", exclude=[], function="keys_products", page=1):
    ...
    if function == "keys_products":
        words = get_true_name(_search_from_product(words))
        if words[0] == 404:
            return words[0]

        if len(exclude) > 0:
            exclude = get_true_name(_search_from_product(exclude))
            if exclude[0] == 404:
                return exclude[0]
